  ...
  follower_name_(follower_name)
{
  ...
  docking_service_server_ = this->create_service<turtlebot3_parallel_follower::srv::StartDocking>(
    service_name,
    std::bind(&Follower::docking_service_callback, this, std::placeholders::_1, std::placeholders::_2));
...
void Follower::docking_service_callback(
  ...
  std::shared_ptr<turtlebot3_parallel_follower::srv::StartDocking::Response> response)
{
  ...
  RCLCPP_INFO(this->get_logger(), "Start docking service called!");

  // For now, just pause the follower timer
  if (send_path_timer_->is_canceled()) {
    RCLCPP_INFO(this->get_logger(), "Follower is already paused.");
    response->success = false;
  } else {
    RCLCPP_INFO(this->get_logger(), "Pausing follower.");
    send_path_timer_->cancel();
