...
class PatrolStopDummy(Node):
    ...
    def __init__(self) -> None:
        ...
        # rear range 모니터(옵션)
        self.rear_range_topic = str(self.declare_parameter('rear_range_topic', '').value)
        ...
        self.rear_update_time: Optional[float] = None
        # 외부 Stop/Go 명령 모니터
        self.last_cmd_state: Optional[bool] = None
        ...
        # rear range 구독(옵션)
        if self.rear_range_topic:
            self.create_subscription(Range, self.rear_range_topic, self._on_rear_range, qos_profile_sensor_data)
            self.get_logger().info(f"rear_range 구독: {self.rear_range_topic}")
        # 외부 Stop/Go 명령 모니터용 구독
        self.create_subscription(Bool, 'patrol/stop', self._on_stop_cmd, 10)
    ...
    # rear range 모니터 콜백
    def _on_rear_range(self, msg: Range) -> None:
        if math.isfinite(msg.range) and msg.min_range <= msg.range <= msg.max_range:
            self.last_rear_range = float(msg.range)
            self.rear_update_time = time.monotonic()
        else:
            self.last_rear_range = None
            self.rear_update_time = None

    # 외부 Stop/Go 명령 모니터 콜백
    def _on_stop_cmd(self, msg: Bool) -> None:
        self.last_cmd_state = bool(msg.data)
    ...
    # rear 모니터 토글
    def toggle_rear_monitor(self) -> None:
        if getattr(self, '_rear_monitoring', False):
            self._stop_rear_monitor()
        else:
            self._start_rear_monitor()

    def _start_rear_monitor(self) -> None:
        if getattr(self, '_rear_monitoring', False):
            return
        self._rear_monitoring = True
        self._rear_monitor_stop = threading.Event()

        def _loop():
            while not self._rear_monitor_stop.is_set():
                now = time.monotonic()
                if self.rear_update_time is not None and (now - self.rear_update_time) <= self.data_timeout and self.last_rear_range is not None:
                    val = f"{self.last_rear_range:.3f} m"
                else:
                    val = 'N/A'
                cmd = 'Stop' if self.last_cmd_state else 'Go' if self.last_cmd_state is not None else 'Unknown'
                print(f"[rear-monitor] rear={val}, cmd={cmd}")
                time.sleep(0.5)

        self._rear_monitor_thread = threading.Thread(target=_loop, daemon=True)
        self._rear_monitor_thread.start()
        print("rear 실시간 조회 시작 (다시 'p' 입력 시 종료)")

    def _stop_rear_monitor(self) -> None:
        if not getattr(self, '_rear_monitoring', False):
            return
        self._rear_monitor_stop.set()
        if getattr(self, '_rear_monitor_thread', None) is not None:
            self._rear_monitor_thread.join(timeout=1.0)
        self._rear_monitor_thread = None
        self._rear_monitoring = False
        print("rear 실시간 조회 종료")
...
def main(args=None) -> None:
    ...
    try:
        while rclpy.ok():
            cmd = input('[s]top / [r]esume / [l]idar check / [c] monitor / [p] rear monitor / [q]uit > ').strip().lower()
            if cmd == 's':
